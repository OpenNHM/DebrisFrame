...
from avaframe.com1DFA.com1DFA import getModuleNames, getSimTypeList, checkCfg, fetchRelVolume, setVolumeIndicator
...
def prepareVarSimDict(standardCfg, inputSimFiles, variationDict, simNameExisting="", module=com1DFA):
    ...
    ### Change begin
    # get max length of dict elements
    max_len = max(len(v) for v in variationDict.values())

    # expand dataframe columns to max length
    variationDictcopy = variationDict.copy()
    for k, v in variationDict.items():
        variationDictcopy[k] = list(v) + v * (max_len - len(v))
        #TODO: adapt code if different relesease scenarios are involved

    # create a dataFrame with given parameter sets
    variationDF = pd.DataFrame(zip(*variationDictcopy.values()), columns=variationDictcopy.keys())
    ### Change end
...
# Overwrite original function from com1DFA.py
